...
impl<W: Write> TagWriter<W> {
    ...
    fn start_tag(&mut self, id: u64, size_length: usize) {
        self.open_tags
            .push((id, Known(self.working_buffer.len()), size_length));
    }
    ...
    fn start_unknown_size_tag(&mut self, id: u64) {
        ...
        self.working_buffer
            .extend_from_slice(&(u64::MAX >> 7).to_be_bytes());
    ...
    /// This function is only used to empty our current working buffer, _without_ closing any tags
    /// automatically. The use case here is for streaming applications.
    pub fn write_current(&mut self) -> Result<(), TagWriterError> {
        self.private_flush()
    }
    ...
    fn write_unsigned_int_tag<const SIZE_LENGTH: usize>(
        &mut self,
        id: u64,
        data: &u64,
    ...
    fn write_signed_int_tag<const SIZE_LENGTH: usize>(
        &mut self,
        id: u64,
        data: &i64,
    ) -> Result<(), TagWriterError> {
    ...
    fn write_utf8_tag<const SIZE_LENGTH: usize>(
        &mut self,
        id: u64,
        data: &str,
    ) -> Result<(), TagWriterError> {
    ...
    fn write_binary_tag<const SIZE_LENGTH: usize>(
        &mut self,
        id: u64,
        data: &[u8],
    ) -> Result<(), TagWriterError> {
    ...
    fn write_float_tag<const SIZE_LENGTH: usize>(
        &mut self,
        id: u64,
        data: &f64,
    ) -> Result<(), TagWriterError> {
    ...
    pub fn write_advanced<TSpec: EbmlSpecification<TSpec> + EbmlTag<TSpec> + Clone>(
        ...
        if options.unknown_sized_element {
            ...
            if should_validate
                && !validate_tag_path::<TSpec>(tag_id, self.open_tags.iter().copied())
            {
